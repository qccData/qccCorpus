...
def api_model(alpha, beta, gamma, layers, s):
    # speech network
    input_speech = Input(shape=(x_train.shape[1], x_train.shape[2]), name='speech_input')
    net_speech = BatchNormalization()(input_speech)
    for i in layers:
      net_speech = LSTM(i, return_sequences=True)(net_speech)
...
model = api_model(1/3, 1/3, 1/3, [512, 256, 128], 'adam')
#model.summary()

earlystop = EarlyStopping(monitor='val_loss', mode='min', patience=10,
                          restore_best_weights=True)
hist = model.fit(x_train, y_train.T.tolist(), batch_size=bs, 
                  validation_data=(x_val, y_val.T.tolist()), epochs=180, verbose=0, shuffle=True, 
                  callbacks=[earlystop])
metrik = model.evaluate(x_test, y_test.T.tolist(), batch_size=bs)
print(metrik)
